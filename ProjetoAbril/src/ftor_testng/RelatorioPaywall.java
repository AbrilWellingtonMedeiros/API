...
public class RelatorioPaywall extends TestUtils {
...
//			 DataDriven.nomeCaso + ".jpg";
			...
			PdfPCell cellc = new PdfPCell(new Paragraph("fechado"));
			PdfPCell celld = new PdfPCell(new Paragraph("contabilizados"));
			...
			for (int i = 0; i < marcas.length; i++) {
				...
				PdfPCell cell7 = null;
				PdfPCell cell8 = new PdfPCell(new Paragraph(FechadosTotal + " = " + fechados));
				PdfPCell cell9 = new PdfPCell(new Paragraph(ContabilizadosTotal + " = " + contabilizados));
				PdfPCell cell10 = new PdfPCell(new Paragraph(String.valueOf(Total)));

			
				Font font = new Font(FontFamily.HELVETICA, 12, Font.BOLD, BaseColor.WHITE);

				if (Double.parseDouble(abertos.replace("%", "").replace(",", ".")) < 5.00) {
					cell7 = new PdfPCell(new Paragraph(AbertosTotal + " = " + abertos));
					cell7.setBackgroundColor(BaseColor.WHITE);
				} else {
					cell7 = new PdfPCell(new Paragraph(AbertosTotal + " = " + abertos, font));
	...
	public void gerarRelatorioPDFErro() throws Exception {
		...
		try {
			...
			// Configura e adiciona as bordas do relatório (seta a cor vermelha,
